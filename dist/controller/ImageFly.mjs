...
export default class ControllerImageFly extends Controller {
    ...
    async action_index() {
        ...
        if (!width && !height)
            throw new Error('Please specify width or height');
        const resizeOption = {};
        if (width)
            resizeOption.width = parseInt(width[1]);
        if (height)
            resizeOption.height = parseInt(height[1]);
        if (fit)
            resizeOption.fit = sharp.fit[fit[1]];
